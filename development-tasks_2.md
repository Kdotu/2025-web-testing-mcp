...
2. ✅ **Playwright 설정 모드 2가지 버전 구현** (Phase 10.1.3 일부) - 항목별 설정 + 시나리오 입력 모드
3. ✅ **테스트 타입 잠금 시스템 완료** (Phase 10.6)
4. ✅ **테스트 타입 ID 통일 완료** ('e2e' → 'playwright')
5. ✅ **기본 MCP 서버 구현 완료** (Lighthouse, k6, Playwright)
6. ✅ **MCP 서버 작동 문제 및 배포 용량 문제 발견 및 분석 완료**
7. ✅ **프론트엔드 테스트 실행/중단 플로우 연결**: `TestExecution.tsx`에서 Lighthouse/Load/Playwright 백엔드 호출 연동, k6 기본 스크립트 자동 생성으로 400 오류 해결
8. ✅ **MCP 도구 파싱 안정화**: Supabase `getMcpTools`가 단일/CSV 문자열도 처리하도록 개선하여 JSON 파싱 오류 제거
