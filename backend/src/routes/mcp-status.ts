...
import { MCPClientFactory } from '../services/mcp-client-factory';
...
router.get('/mcp-status', async (_req, res) => {
  try {
    ...
    const serverStatus = await MCPClientFactory.checkExternalServers();
...
router.post('/mcp-status/test', async (req, res) => {
  try {
    ...
    switch (serverType) {
      case 'k6':
        client = MCPClientFactory.createExternalK6Client();
      ...
      case 'lighthouse':
        client = MCPClientFactory.createExternalLighthouseClient();
      ...
      case 'playwright':
        client = MCPClientFactory.createExternalPlaywrightClient();
