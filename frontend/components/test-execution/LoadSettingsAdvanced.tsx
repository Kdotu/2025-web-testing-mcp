...
interface LoadSettingsAdvancedProps {
  loadSettings: any;
  update: (key: string, value: any) => void;
}

export function LoadSettingsAdvanced({ loadSettings, update }: LoadSettingsAdvancedProps) {
  ...
  const handlePresetChange = (presetId: string) => {
    const preset = loadTestPresets.find(p => p.id === presetId);
    if (!preset) return;
    update("preset", presetId);
    update("startRate", preset.startRate);
    update("timeUnit", preset.timeUnit);
    update("preAllocatedVUs", preset.preAllocatedVUs);
    update("maxVUs", preset.maxVUs);
    update("stages", [...preset.stages]);
  };
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="neu-subtle rounded-xl px-6 py-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            {loadTestPresets.map((preset) => {
              ...
              const isSelected = loadSettings?.preset === preset.id;
              return (
                <button
                  key={preset.id}
        ...
        <div className="neu-subtle rounded-xl px-6 py-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="neu-pressed rounded-lg px-3 py-1">
              <span className="text-xs text-muted-foreground">Executor: {loadSettings?.executor || "ramping-vus"}</span>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div className="neu-pressed rounded-lg px-4 py-3">
              ...
              <div className="flex flex-col">
                <span className="text-foreground font-semibold">{loadSettings?.startRate || 0} 요청 / {loadSettings?.timeUnit || "1s"}</span>
                <span className="text-xs text-foreground">≈ {Math.round(((loadSettings?.startRate || 0) / parseInt(loadSettings?.timeUnit || "1")) * 100) / 100} TPS</span>
              </div>
            </div>
            <div className="neu-pressed rounded-lg px-4 py-3">
              ...
              <span className="text-foreground font-semibold">{loadSettings?.timeUnit || "20s"}</span>
            </div>
            <div className="neu-pressed rounded-lg px-4 py-3">
              ...
              <span className="text-foreground font-semibold">{loadSettings?.preAllocatedVUs || 10}</span>
            </div>
            <div className="neu-pressed rounded-lg px-4 py-3">
              ...
              <span className="text-foreground font-semibold">{loadSettings?.maxVUs || 500}</span>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-6">
          <div className="neu-subtle rounded-xl px-6 py-6">
            ...
            <div className="neu-input rounded-xl px-3 py-2">
              <Select value={loadSettings?.executor || "ramping-vus"} onValueChange={(value) => update("executor", value)}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <div className="neu-input rounded-lg px-3 py-2 ring-1 ring-primary/20">
                  <Input type="number" value={loadSettings?.startRate || 100} onChange={(e) => {
              ...
              <div>
                ...
                <div className="neu-input rounded-lg px-2 py-1 ring-1 ring-primary/20">
                  <Select value={loadSettings?.timeUnit || "20s"} onValueChange={(value) => update("timeUnit", value)}>
              ...
              <div>
                ...
                <div className="neu-input rounded-lg px-3 py-2 ring-1 ring-primary/20">
                  <Input type="number" value={loadSettings?.preAllocatedVUs || 10} onChange={(e) => {
              ...
              <div>
                ...
                <div className="neu-input rounded-lg px-3 py-2 ring-1 ring-primary/20">
                  <Input type="number" value={loadSettings?.maxVUs || 500} onChange={(e) => {
      ...
      <div className="neu-subtle rounded-xl px-6 py-6">
        <Label className="text-foreground font-semibold text-lg mb-4 block">테스트 단계 <span className="text-xs text-muted-foreground ml-1">(Stages)</span>{loadSettings?.preset !== "custom" && (<span className="text-sm text-muted-foreground ml-2">• 프리셋 기반</span>)}</Label>
        <div className="space-y-4">
          {(loadSettings?.stages || []).map((stage: LoadTestStage, index: number) => (
            <div key={`stage-${index}-${stage.target}-${stage.duration}`} className="neu-input rounded-xl p-4">
              <div className="grid grid-cols-3 gap-4 items-center">
                <div>
                  <Label className="text-sm text-muted-foreground mb-2 block">목표 요청 수 <span className="text-xs text-muted-foreground">(per {loadSettings?.timeUnit || "20s"})</span></Label>
                  <Input type="number" value={stage.target || 0} onChange={(e) => {
                    if (loadSettings?.preset !== "custom") return;
                    const newStages = [...(loadSettings?.stages || [])];
                    newStages[index] = { ...stage, target: Number(e.target.value) || 0 };
                    update("stages", newStages);
                  }} disabled={loadSettings?.preset !== "custom"} className={`border-none bg-transparent text-foreground text-center font-semibold ${loadSettings?.preset !== "custom" ? "opacity-60" : ""}`} min="0" max="50000" />
                </div>
                <div>
                  <Label className="text-sm text-muted-foreground mb-2 block">지속 시간</Label>
                  {loadSettings?.preset === "custom" ? (
                    <Select value={stage.duration || "1m"} onValueChange={(value) => {
                      const newStages = [...(loadSettings?.stages || [])];
